...
class CustomEnv(gym.Env):
  ...
  def __init__(self, load, set = None):
    ...
    self.observation_space = spaces.Box(low=np.array([0, 0]), high=np.array([24, 999]))
  ...
  def step(self, action):
    ...
    if done:
      self.hour -= 1
    observation = [self.hour, self.price[self.hour]]
  ...
  def reset(self):
    ...
    self.hour = 0
    observation = [self.hour, self.price[0]]
...
set_seeds(22373)
electricVehicle = ShiftableInterruptible(toml_dict["SI"][0]["id"], toml_dict["SI"][0]["P"], toml_dict["SI"][0]["ts"], toml_dict["SI"][0]["tf"], toml_dict["SI"][0]["L"])
env = CustomEnv(electricVehicle,  "training")
...
for episode in range(1500):
    ...
    print("training EV" , "episode", episode)
    env.reset()
    model.learn(total_timesteps=24)
    validationTotalCost = 0
    #then check cost on validation set
    for validationScenario in range(10):
        electricVehicleValidation = ShiftableInterruptible(toml_dict["SI"][0]["id"], toml_dict["SI"][0]["P"], toml_dict["SI"][0]["ts"], toml_dict["SI"][0]["tf"], toml_dict["SI"][0]["L"])
        envValidation = CustomEnv(electricVehicleValidation, "validation")
    ...
    np.savetxt('PPO_EV_22373_validationCost.txt', np.asarray(validationCostArray), delimiter=',')
    model.save("PPO_EV_22373")
